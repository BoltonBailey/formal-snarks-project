...
This file contains the completeness proof for the Lipmaa SNARK (the Groth16 variant with the
toxic waste Kronecker-packed into powers of a single sample `y`: α = y^75, β = y^25, γ = y^5,
δ = y^1). The honest prover and the argument are those of the Type III Groth16 completeness
proof.
...
lemma is_complete
  ...
  · intro check_idx
    ...
    -- Under the Kronecker packing (γ = y^5, δ = y^1) the Type III multiplier γ²δ² becomes y^12
    linear_combination ((X (some Vars.y) : MvPolynomial (Option Vars) F) ^ 12) * hkey

...
The SNARK is defined over computable polynomials (`CPoly.CMvPolynomial`/`CompPoly.CPolynomial`).
The proof proceeds by transporting the verification equation to mathlib's `MvPolynomial` via the
`CPoly.polyRingEquiv` ring equivalence, then viewing `MvPolynomial (Option Vars) F` as polynomials
